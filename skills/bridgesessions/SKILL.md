...
- **Current public release: `v2.0.15-alpha6`** (multi-platform alpha on Codeberg).
...
## Public release (`v2.0.15-alpha6`)

| Fact | Value |
|------|--------|
| Tag | `v2.0.15-alpha6` (2026-07-26) |
| Branch | `main` |
...
| Notes | `docs/RELEASE-NOTES-2.0.15-alpha6.md` · provenance `docs/RELEASE-PROVENANCE.md` |

### New in v2.0.15-alpha6

- **Warning hygiene:** `join` + daemonize paths check return values (save_config,
  ensure_private_directory, system, freopen×3); host auto-authorize now reports
  persistence failures via `JoinReplyMsg.error` instead of ghost-"ok" (the
  join-ok-then-`hello_rejected` pattern). Builds clean of `-Wunused-result`.
- Carries everything from v2.0.14-alpha6: file-xfer double-compression fix, properly
  static Linux dist, BridgePanel 10-tab fleet dashboard, screen/video capture.
...
Tree: https://codeberg.org/Mind-Dragon/BridgeSessions/src/tag/v2.0.15-alpha6/dist
...
- Re-tag after changing `dist/`: `git tag -f v2.0.15-alpha6 HEAD && git push --force codeberg main && git push --force codeberg v2.0.15-alpha6`.
...
git push codeberg v2.0.15-alpha6
```
...
bs --version                          # expect 2.0.15-alpha6
...
## Security posture (v2.0.15-alpha6)
...
./build/bridgesessions --version   # 2.0.15-alpha6
...
- Release notes: `docs/RELEASE-NOTES-2.0.15-alpha6.md`
...
curl -fsI https://codeberg.org/Mind-Dragon/BridgeSessions/raw/tag/v2.0.15-alpha6/dist/bridgesessions-linux-x86_64
```
