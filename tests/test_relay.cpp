...
TEST_CASE("AttachMsg creates session and sets attached_session on conn", "[relay][attach]") {
    ...
    const std::string profile_command = cfg.default_shell;
    cfg.default_shell = "definitely-not-the-profile-command";
    cfg.session_commands["my-shared-session"] = profile_command;
    MeshController mc(cfg);
    ...
    REQUIRE(s2->command == profile_command);
...
#ifdef _WIN32
    // ResizePseudoConsole has no public size getter; exercising it without error
    // is the available contract on Windows.
    REQUIRE(true);
#else
    winsize ws{};
...
TEST_CASE("Restart signal preserves replacement command and generation",
          "[relay][signal][restart]") {
    auto cfg = make_test_config("test-node-restart");
    MeshController mc(cfg);
    ...
    auto* s = mc.sessions().attach("restart-test", initial, 80, 24,
                                   "xterm-256color");
    REQUIRE(s != nullptr);
    REQUIRE(s->is_valid());
    ...
    sc.conn.attached_session = s;
    SignalMsg sig;
    sig.signal = SignalMsg::SignalType::Restart;
    sig.process = replacement;
    Message msg = sig;

    mc.handle_inbound_session(sc.conn, msg);

    const std::string expected_replacement = prepare_session_command(
        {replacement, SessionCommandSource::ClientOverride});
    REQUIRE(s->is_valid());
