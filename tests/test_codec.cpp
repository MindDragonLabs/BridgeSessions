...
TEST_CASE("Round-trip: HelloMsg", "[codec][roundtrip]") {
    ...
    SECTION("HelloMsg with multiple peers") {
        ...
        m.version = "2.0.0";
        m.pubkey_hex = "key123";
        m.known_peers.push_back(PeerInfo{"linux-a", "203.0.113.11:19948", "key-linux-a", 1000000});
...
TEST_CASE("Round-trip: GossipMsg", "[codec][roundtrip]") {
    SECTION("basic GossipMsg") {
        GossipMsg m;
        m.peers.push_back(PeerInfo{"shadow", "203.0.113.11:19948", "abc123", 1000});
        m.peers.push_back(PeerInfo{"linux-a", "100.112.19.19:19948", "def456", 2000});
        auto m2 = roundtrip(m);
        REQUIRE(m2 == m);
    }
