...
- `scripts/nl2sh-hub-setup.sh` installs whatisit-nl2sh + optional loopback HTTP on Linux mesh hubs.
...
- **bs-qa-ubuntu** KVM guest on Linux hub for automated Linux CUA/display; mesh via virbr0.
- Runner always includes **Windows peer** in the peer matrix.
- **Desktop e2e framework** (`docs/E2E-FRAMEWORK.md`, `tests/e2e/`): L2 mesh + L3
  CUA/tray/menubar orchestrator; Windows helper bootstrap; KVM Ubuntu guest on Linux hub;
...
- **`scripts/e2e-fleet-test.sh`**: live multi-platform fleet e2e matrix (health,
  ...
  health). Defaults: `Linux hub`/`Linux hub` (Linux), `macOS peer` (macOS), `Windows peer`
