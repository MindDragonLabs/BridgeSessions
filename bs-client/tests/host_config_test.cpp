...
TEST_CASE("HostConfig: parses ssh-like aliases", "[host_config]") {
    ...
    std::ofstream(path) << "# comment\n"
                        << "test-pc1 203.0.113.10:9948 key=/tmp/k cert=/tmp/c session=work\n"
                        << "local 127.0.0.1:9947\n";

    auto hosts = load_hosts_file(path.string());
    REQUIRE(hosts.size() == 2);
    REQUIRE(hosts[0].name == "test-pc1");
    REQUIRE(hosts[0].server == "203.0.113.10:9948");
...
TEST_CASE("HostConfig: resolves positional alias like ssh test-pc1", "[host_config]") {
    std::vector<HostEntry> hosts{
        HostEntry{.name="test-pc1", .server="203.0.113.10:9948", .key_file="/keys/k", .cert_file="/keys/c", .session_name="hermes"}
    };
    ConnectionOptions opts;
    opts.server = "127.0.0.1:9948";
    opts.session_name = "default";

    auto err = apply_target_and_hosts("test-pc1", hosts, opts);
    REQUIRE_FALSE(err.has_value());
    REQUIRE(opts.server == "203.0.113.10:9948");
...
TEST_CASE("HostConfig: upsert and remove host entries", "[host_config]") {
    ...
    REQUIRE(upsert_host(path.string(), HostEntry{.name="test-pc1", .server="203.0.113.10:9948"}));
    REQUIRE(upsert_host(path.string(), HostEntry{.name="test-pc1", .server="127.0.0.1:9947", .session_name="smoke"}));
    ...
    REQUIRE(remove_host(path.string(), "test-pc1"));
